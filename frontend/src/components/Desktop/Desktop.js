...
const Desktop = () => {
  ...
  const handleIconDoubleClick = (iconId) => {
    ...
    } else if (iconId === 'readme') {
      openWindow(
        'readme',
        'text',
        'README.txt',
        <div style={{ padding: '10px', whiteSpace: 'pre-line', fontFamily: 'monospace' }}>
          Welcome to Windows Media Explorer!
          
          This application lets you browse, manage and interact with video content through a nostalgic Windows 95/98 interface.
          
          Getting Started:
          1. Double-click on "My Computer" to open the file browser
          2. Browse through Videos, Audio, and Images folders
          3. Double-click on any media to play it
          4. Use the context menu (right-click) for more options
          
          Note: You need to connect to VideoDB by setting up the backend with your API key.
          
          Enjoy your nostalgic media browsing experience!
        </div>,
        { 
          icon: ICONS.FILE_TYPES.UNKNOWN,
  ...
  const handleTaskItemClick = (windowId) => {
  ...
  // Handle open My Computer directly for demonstration purposes
  const handleOpenMyComputer = () => {
    handleIconDoubleClick('my-computer');
  };
